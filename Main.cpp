...
void gl_init(){
	...
	Image_ptr layer2 = Image_ptr(new Image(
						...
						//Image::createRandomBicolorNoise(2*imageWidth,2*imageHeight)
						Image::createPerlinNoise(2*imageWidth,2*imageHeight,
								Color::BLACK,Color::WHITE,1./10 /*scale*/,0./*time*/)
						));
	...
	stack.push(layer2,Image::createSolidColorMask(2*imageWidth,2*imageHeight,0.2));


	//add to stack
	img = stack.getCondensedImage();
	/** Here it ends and i copy it to an array **/
