...
template <int SIZE>
class PerlinNoise3{//should be a 2^n, for integral number n > 1
	...
	public:
		...
		double   noise(double x, double y, double z){
			int   	X = (int)floor(x) & (SIZE/2-1),             /* FIND UNIT CUBE THAT */
      				Y = (int)floor(y) & (SIZE/2-1),             /* CONTAINS POINT.     */
      				Z = (int)floor(z) & (SIZE/2-1);
...
int main(){
	PerlinNoise3<512> p;
	double max = 0 , min = 0;
	for(int u=0; u< p.getSize()/2; ++u)
		for(int i=0; i< p.getSize()/2; ++i)
			for(int j=0; j< p.getSize()/2; ++j){
				double n = p.noise(i+0.5,j+0.5,u+0.5);
				max = std::max(max,n);
				min = std::min(min,n);
			}
...
/*** here starts the given C implementation ***/
//#include <math.h>
//#include <stdio.h>
//
//static int p[512];
